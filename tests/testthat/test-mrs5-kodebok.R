...
kb_felter_raa = list(
  kb_test_felter_raa = tibble::tibble(
    Skjemanavn = c(rep("Testskjema", 10)), 
    Variabelnavn = c("PasientGUID", "Skjematype", "UnitId", "PatientAge", 
                     "CreationDate", "Innlagt", "FormStatus", 
                     "PatientGender", "Hoyde", "HoydeUkjent"),
    Visningstekst = c("PasientGUID", "Skjematype", "UnitId", "PatientAge", 
                      "Opprettet", "Innlagt", "FormStatus", 
                      "PatientGender", "Høyde (cm)", "Høyde ukjent"),
    `Unik teknisk referanse` = c("PatientInRegistryGuid", "FormTypeName", 
                                 "UnitId", "PatientAge", "CreationDate", 
                                 "Innlagt", "FormStatus", "PatientGender", 
                                 "Hoyde", "HoydeUkjent"),
    `Mulige verdier` = c(NA, NA, NA, NA, NA, NA, 
                         "0 = Ingen, 1 = Kladd, 2 = Ferdigstilt, 4 = Slettet, 5 = Returnert", 
                         "0 = Ukjent, 1 = Mann, 2 = Kvinne", NA, NA),
    Felttype = c("String", "String", "Number", "Number", "DateTime", 
                 "DateTime", "Enum", "Enum", "Number", "Bool"),
    Kodeverk = c(rep(NA_character_, 10)), 
    Kjernefelt = c("Ja", "Ja", "Ja", "Ja", "Ja", "Ja", 
                   "Ja", "Ja", "Nei", "Nei"),
    Identifiserbar = c("Nei", "Nei", "Nei", "Nei", "Nei", 
                       "Nei", "Nei", "Nei", NA, NA),
    `Gyldig fra og med skjemaversjon` = c("1", "1", "1", "1", "1", 
                                          "1", "1", "1", "10", "10"),
    `Fjernet fra og med skjemaversjon` = c(rep(NA_character_, 10)),
    `Tillat kun positive verdier` = c(rep(NA_character_, 10)),
    Hjelpetekst = c(rep(NA_character_, 10)),
    `Skal vises i registeruttrekk` = c(rep("Ja", 10)),
    `Skal eksporteres til Hap Helsedata` =  c(rep("Ja", 10)),
    `Gammelt navn` = c(rep(NA_character_, 10)),
    Beskrivelse = c("GUID til pasienten, ulik lokalt og nasjonalt", 
                    ...
                    "Dato og tidspunkt skjemaet ble opprettet", 
                    "Dato og tidspunkt pasienten ble innlagt", "Skjemaets status", 
                    "Pasientens kjønn gitt fra Personregisteret", NA, NA),
    `Patientvennlig term` = c(rep(NA_character_, 10)),
    Tema = c(rep(NA_character_, 10)),
    Kommentar = c(rep(NA_character_, 10)),
    `Intern kommentar` = c(rep(NA_character_, 10)),
    Feltkilde = c("Calculated", "Calculated", "Calculated", "Calculated", 
                  "Calculated", "DirectFromForm", 
