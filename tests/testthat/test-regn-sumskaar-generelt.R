...
st_eks_flere_konstantledd = rbind(
  st_eks,
  tibble(
...
test_that("skaar_datasett() fungerer med skåringstabell med konstantledd", {
  expect_equal(
    object = skaar_datasett(d_gyldig_inn,
      skaaringstabell = st_eks_flere_konstantledd
    ),
...
test_that("skaar_datasett() fungerer hvis en av de to sumskår-kolonnene
  ...
  d_ut_fasit = d_inn_inkl_1_sumskaar
  d_ut_fasit$total = c(0.518, 0.775, 1.14)
  d_ut_fasit = add_column(
    d_ut_fasit,
...
test_that("skaar_datasett() gir ut feilmelding hvis skåringstabell,
  ...
  st_ugyldig = st_eks
  st_ugyldig$verdi[2:3] = 6
  expect_error(skaar_datasett(d_gyldig_inn,
    skaaringstabell = st_ugyldig
  ))
...
test_that("sjekk_skaaringstabell() gir ingen feilmelding hvis
  ...
  expect_silent(sjekk_skaaringstabell(st_eks))
})
...
test_that("sjekk_skaaringstabell() gir feilmelding hvis skåringstabellen
  ...
  st_ugyldig_navn_delskala = st_eks
  st_ugyldig_navn_delskala = rename(
    st_ugyldig_navn_delskala,
    deelskala = delskala
  )
  expect_error(
    sjekk_skaaringstabell(st_ugyldig_navn_delskala),
    feilmelding_kolonnenavn
  )

  st_ugyldig_navn_variabel = st_eks
  st_ugyldig_navn_variabel = rename(
    st_ugyldig_navn_variabel,
    var = variabel
  )
  expect_error(
    sjekk_skaaringstabell(st_ugyldig_navn_variabel),
    feilmelding_kolonnenavn
  )

  st_ugyldig_navn_verdi = st_eks
  st_ugyldig_navn_verdi = rename(
    st_ugyldig_navn_verdi,
    tallverdi = verdi
  )
  expect_error(
    sjekk_skaaringstabell(st_ugyldig_navn_verdi),
    feilmelding_kolonnenavn
  )

  st_ugyldig_navn_koeffisient = st_eks
  st_ugyldig_navn_koeffisient = rename(
    st_ugyldig_navn_koeffisient,
    koeff = koeffisient
  )
  expect_error(
    sjekk_skaaringstabell(st_ugyldig_navn_koeffisient),
    feilmelding_kolonnenavn
  )
})
...
test_that("sjekk_skaaringstabell() gir feilmelding hvis skåringstabellen
          ...
          samme delskala", {
  st_ugyldig_dupl_verdi = st_eks
  st_ugyldig_dupl_verdi$verdi[1:2] = 1

  expect_error(
    sjekk_skaaringstabell(st_ugyldig_dupl_verdi),
...
st_manglende_oppforinger = tribble(
  ~delskala, ~variabel, ~verdi, ~koeffisient,
  "fys", "var_a", 1, 0,
  "fys", "var_a", 2, 0,
  "psyk", "var_a", 1, 0,
...
test_that("sjekk_skaaringstabell() gir feilmelding hvis en variabel
  ...
  expect_error(sjekk_skaaringstabell(st_manglende_oppforinger),
    paste0(
...
test_that("sjekk_skaaringstabell() gir ingen feilmelding hvis en variabel
  ...
  st_na_i_1_delskala = st_manglende_oppforinger
  st_na_i_1_delskala$verdi[2] = NA

  expect_silent(sjekk_skaaringstabell(st_na_i_1_delskala))
})
...
test_that("sjekk_skaaringstabell() gir feilmelding hvis en rad har verdien NA
  ...
  st_ugyldig_na_variabel = add_row(st_eks,
    delskala = "total", variabel = NA, verdi = 10, koeffisient = 10
  )
  expect_error(
    object = sjekk_skaaringstabell(st_ugyldig_na_variabel),
...
test_that("sjekk_skaaringstabell() gir feilmelding hvis flere rader har verdien
  ...
  st_ugyldig_na_variabel_dobbel = st_eks |>
    rbind(tibble(
      delskala = "total", variabel = NA, verdi = NA, koeffisient = c(10, 5)
    ))
  expect_error(
    sjekk_skaaringstabell(st_ugyldig_na_variabel_dobbel)
  )
})

test_that("sjekk_skaaringstabell() gir feilmelding hvis
          koeffisient-kolonnen i skåringstabellen inneholder NA-verdier", {
  st_ugyldig_na_koeffisient = st_eks
  st_ugyldig_na_koeffisient$koeffisient[2] = NA

  expect_error(
    sjekk_skaaringstabell(st_ugyldig_na_koeffisient),
...
test_that("sjekk_skaaringstabell() gir feilmelding hvis
  ...
  st_ugyldig_na_delskala = add_row(st_eks,
    delskala = NA, variabel = NA, verdi = NA, koeffisient = 10
  )

  expect_error(
    sjekk_skaaringstabell(st_ugyldig_na_delskala),
...
test_that("sjekk_skaaringstabell() gir feilmelding hvis skåringstabellen
  ...
  st_ugyldig_verdi_kolonne = st_eks
  st_ugyldig_verdi_kolonne$verdi = as.character(
    st_ugyldig_verdi_kolonne$verdi
  )
  expect_error(
    sjekk_skaaringstabell(st_ugyldig_verdi_kolonne),
    feilmelding_kolonneformat
  )

  st_ugyldig_koeffisient_kolonne = st_eks
  st_ugyldig_koeffisient_kolonne$koeffisient = as.character(
    st_ugyldig_koeffisient_kolonne$koeffisient
  )
  expect_error(
    sjekk_skaaringstabell(
      st_ugyldig_koeffisient_kolonne
    ),
    feilmelding_kolonneformat
  )

  st_ugyldig_variabel_kolonne = st_eks
  st_ugyldig_variabel_kolonne$variabel = seq_len(nrow(
    st_ugyldig_variabel_kolonne
  ))
...
st_ugyldige_kolonnenavn = rename(st_eks,
  tullball = variabel,
  ballball = verdi
)

# Eksempel på skåringstabell med ugyldig format
st_ugyldig_format = table(st_eks)

# Eksempel på skåringstabell med både ugyldige kolonnenavn og ugyldig
# format
st_ugyldig_navn_og_format = table(
  st_ugyldige_kolonnenavn
)
...
test_that("sjekk_variabelverdier() gjev inga feilmelding for datasett
  ...
  expect_silent(sjekk_variabelverdier(d_gyldig_eks1, st_eks,
    godta_manglende = FALSE
  ))
  expect_silent(sjekk_variabelverdier(d_gyldig_eks2, st_eks,
    godta_manglende = FALSE
  ))
})
...
test_that("sjekk_variabelverdier() gir ingen feilmelding for datasett
  ...
  expect_silent(sjekk_variabelverdier(d_ugyldig_na, st_eks,
...
test_that("sjekk_variabelverdier() gjev feilmelding for datasett med
  ...
  expect_error(sjekk_variabelverdier(d_ugyldig_na, st_eks,
    godta_manglende = FALSE
  ))
})
...
test_that("oppsummer_ugyldige_verdier() presenterer korrekte feilverdier
  ...
  expect_identical(
    oppsummer_ugyldige_verdier(ugyldigtab_2_feil),
    "Fant 3 ugyldige verdier:\ngen: 9, 6\npsyk2: NA"
  )
  expect_identical(
    oppsummer_ugyldige_verdier(ugyldigtab_2_like_feil),
    "Fant 3 ugyldige verdier:\ngen: 4, 4\npsyk2: NA"
  )
})
...
test_that(paste0(
  ...
  ugyldigtab_2_feil_omsortert = ugyldigtab_2_feil
  ugyldigtab_2_feil_omsortert =
    ugyldigtab_2_feil_omsortert[c(2, 1, 3), ]
  expect_identical(
    oppsummer_ugyldige_verdier(ugyldigtab_2_feil),
    oppsummer_ugyldige_verdier(ugyldigtab_2_feil_omsortert)
  )
})
...
# Éin verdi per rad i datasettet, i same rekkjefølgje som radene der.
fasit_total = c(
  0.2 + 0.3 + 0 + 0 + 0.018,
  0.4 + 0 + 0.35 + 0.025 + 0,
  0.8 + 0 + 0.35 + (-0.01) + 0
...
sumskaar_tabell = tibble(total = fasit_total, psykisk = fasit_psykisk)
...
# Berre «total» for første rad vert påverka; resten er som i fasiten.
sumskaar_na_tabell = tibble(
  total = c(NA_real_, fasit_total[2:3]),
  psykisk = fasit_psykisk
)

test_that("skaar_datasett_uten_validering() gir ut NA som sumskår ved
          NA-verdier uten tilknyttet koeffisient", {
  expect_equal(skaar_datasett_uten_validering(d_na, st_eks),
...
d_1_bare_na = d_gyldig_alle_verdier
d_1_bare_na[1, ] = NA

# Utregnede sumskårer for eksempeldata hvor 1 av besvarelsene har
# NA-verdier på alle spørsmål.
# Begge sumskårane for første rad vert NA; resten er som i fasiten.
sumskaar_1_bare_na_tabell = tibble(
  total = c(NA_real_, fasit_total[2:3]),
  psykisk = c(NA_real_, fasit_psykisk[2:3])
)
...
test_that("skaar_datasett_uten_validering() gir ut riktig sumskår hvis 1
  ...
  expect_equal(
    ...
    expected = sumskaar_1_bare_na_tabell,
    tolerance = testthat_tolerance()
  )
})

# Eksempeldata hvor alle besvarelsene har NA-verdier på alle spørsmål
d_alle_bare_na = d_gyldig_alle_verdier
d_alle_bare_na[] = NA

# Utregnede sumskårer for eksempeldata hvor alle besvarelsene har
# NA-verdier på alle spørsmål
sumskaar_alle_bare_na_tabell = tibble(
  total = rep(NA_real_, 3),
  psykisk = rep(NA_real_, 3)
)
...
test_that("skaar_datasett_uten_validering() gir ut riktige sumskårer
  ...
  expect_identical(
    ...
    expected = sumskaar_alle_bare_na_tabell
  )
})
...
# Utregnede sumskårer for eksempeldata som bare inneholder 1
# besvarelse (rad 2 i det opphavlege datasettet)
sumskaar_1_besvarelse_tabell = tibble(
  total = fasit_total[2],
  psykisk = fasit_psykisk[2]
)
...
test_that("skaar_datasett_uten_validering() gir ut riktige sumskårer i
  ...
  expect_equal(
    object = skaar_datasett_uten_validering(d_enkelt_eks_inn,
      skaaringstabell = st_flere_delskalaer
    ),
...
test_that("skaar_datasett_uten_validering() gir ut riktige sumskårer
  ...
  st_eks_ett_konstantledd = add_row(st_eks,
    delskala = "total", variabel = NA, verdi = NA, koeffisient = 1
  )
  sumskaar_ett_konstantledd = sumskaar_tabell
  sumskaar_ett_konstantledd$total =
    sumskaar_ett_konstantledd$total + 1
  expect_equal(
    object = skaar_datasett_uten_validering(
      d_gyldig_alle_verdier,
      st_eks_ett_konstantledd
    ),
    expected = sumskaar_ett_konstantledd,
    tolerance = testthat_tolerance()
  )
})
...
test_that("skaar_datasett_uten_validering() gir ut riktige sumskårer
  ...
  sumskaar_flere_konstantledd = sumskaar_tabell
  sumskaar_flere_konstantledd$total =
    sumskaar_flere_konstantledd$total + 1
  sumskaar_flere_konstantledd$psykisk =
    sumskaar_flere_konstantledd$psykisk + 5
  expect_equal(
    object = skaar_datasett_uten_validering(
      d_gyldig_alle_verdier,
      st_eks_flere_konstantledd
    ),
    expected = sumskaar_flere_konstantledd,
    tolerance = testthat_tolerance()
  )
})
...
test_that("legg_til_na_i_skaaringstabell() fungerer", {
  ...
  expect_identical(
    legg_til_na_i_skaaringstabell(st_orig),
    st_med_na
  )
  ...
  expect_identical(
    legg_til_na_i_skaaringstabell(st_orig),
    legg_til_na_i_skaaringstabell(legg_til_na_i_skaaringstabell(st_orig))
  )
  expect_identical(
    legg_til_na_i_skaaringstabell(st_eks),
    legg_til_na_i_skaaringstabell(legg_til_na_i_skaaringstabell(st_eks))
  )
})
...
test_that("legg_til_na_i_skaaringstabell() overskriv ikkje eksisterande
  ...
  expect_identical(
    legg_til_na_i_skaaringstabell(st_med_na),
    st_med_na
  )
})
