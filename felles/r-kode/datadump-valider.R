...
kb_rename = kb %>% rename(varnamn = "varabel_id")
kb_num = kb_rename %>% filter(variabeltype == "numerisk" | variabeltype == "kategorisk" | variabeltype == "utrekna") %>% distinct(varnamn)
kb_boolsk = kb_rename %>% filter(variabeltype == "boolsk") %>% select(varnamn)
kb_tekst = kb_rename %>% filter(variabeltype == "tekst") %>% select(varnamn)
...
regelsett = list(er_innfor_min_og_maks,
                 har_riktig_ant_des,
...
regelsett = lag_regelsett(kb, oblig = TRUE, min = min_verdi)
