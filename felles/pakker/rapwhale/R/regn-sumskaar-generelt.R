...
sjekk_skaaringstabell = function(skaaringstabell) {
  ...
  if (!all(hasName(skaaringstabell, c("delskala", "variabel", "verdi", 
                                      "koeffisient")))) {
  ...
  if (nrow(d_ugyldige) > 0) {
    oppsummert = d_ugyldige %>%
      ...
      summarise(feil_variabler_verdier = paste0(variabel, ": ", 
                                                feil_verdier, 
                                                collapse="\n")) %>% 
      summarise(feiltekst = paste0("Fant ", nrow(d_ugyldige), 
                                   " ugyldige verdier:\n", 
...
legg_til_eller_erstatt_kolonner = function(d_orig, d_ekstrakol) {
  ...
  if (length(navn_finst) > 0) {
    warning("Følgende kolonne(r) i datasettet har blitt overskrevet: ", 
            navn_finst_tekst)
  }
