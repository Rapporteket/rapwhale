...
Me har tidlegare vist eksempel på enkelt tilfeldig utval og
ulike former stratifiserte utval.
Me skal no sjå nærare på korleis me kan,
og ikkje kan,
estimera dei ulike storleikane for dei ulike samplingsmetodane.
...
* Prosentdelen av éin type måling (eks. biverknadar) som er rett
  i heile registeret.
* Prosentdelen målingar per sjukehus som er rette.
* Prosentdelen av éin type måling som er rett per sjukehus.

Estimata vert forventingsrette og konfindensintervalla gyldige.

Denne samplingsmetoden er den mest enkle og fleksible,
då han kan gje svar på *alle* moglege problemstillingar,
men han har òg nokre ulemper.

La oss no tenkja oss at me har eit register med tre sjukehus
...
Kvar pasient er registrert me `r `n_var` variablar.
...
Dette kan me bruka direkte for å estimera kor stor del
av målingane som er korrekte per sjukehus,
for å samanlikna sjukehusa (gjerne med SPC-metodikk):
...
`paste0(n_korrekte, " / ", n_trekte, " = ",
        format(round(n_korrekte/n_trekte, 2), nsmall=2))`.
Men dette svaret er openbert feil.
Dei dårlege resultata frå sjukehus A har fått altfor stor vekt.

Metoden ovanfor vil berre vera gyldig dersom det ikkje finst
...
målingane i *heile registeret* som er rette,
vert då cirka `r round(n_bra_estimat, 2)`.
Dette estimatet er nærare på estimatetet til sjukehus C,
sidan dette har dei fleste pasientane/målingane i registeret.

Utrekning av tilhøyrande konfidensintervall vert meir komplisert.
Viss estimata ikkje er veldig små
og ikkje er baserte på så veldig få observasjonar,
