...
Men korleis kan me sikra at me ikkje prøver å validera unødvendige verdiar?
Her er tre løysingar:

- Ignorer heile problemstillinga.
- Trekk tilfeldige rader/målingar som før,
  men sjå vekk frå undervariablane dersom
  hovudvariabelen ikkje er `TRUE`.
- Behandla variablane som eit «sett».
...
d_vld_n_med_biverk = full_join(d_vld, d_pasientar_biverk, by = names(d_vld))
```
...
d_vld_prioritert = bind_rows(d1_uttrekk, d2_uttrekk) %>%
  lag_valideringsdatasett(indvars = c("pasid", "dato_inn"))
```
