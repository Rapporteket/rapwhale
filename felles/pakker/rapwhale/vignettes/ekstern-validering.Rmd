...
Ekstern validering går ut på å finna korrektheita til eit register, altså å 
sjekka om dataa i registeret er korrekte. I praksis vert dette gjort ved å 
samanlikna registerdata med ein gullstandard, typisk pasientjournal.
...
d_superenkelt %>%
  kbl(align = c("llrr")) %>%
  kable_classic("hover", full_width = F, position = "left")
```
...
Dersom ein ser vidare på meir realistiske datasett med fleire variablar og 
registreringar vil ein oppdaga utfordringar som formatet i dette enkle 
eksempelet ikkje løyser. Det endelege formatet må difor verta litt meir 
komplisert.

Eit problem med dette formatet er dersom same pasient har fleire 
registreringar på same variabel. Kanskje pasienten med pasientid 5 har 
registrert vekt til dømes før og etter operasjon. Datasettet vil då ha to 
rader med `pasid = 5` og `varnamn = "vekt"`, så når ein skal fylla inn dei to 
verdiane frå journal veit ein ikkje kva for ein som skal kor. 

For å løysa dette problemet treng ein fleire *indeksvariablar* i tillegg til 
`pasid` og `varnamn`, slik at kombinasjonen av desse unikt identifiserer kvar 
rad. Kombinasjonen av indeksvariablane skal altså vera primærnøkkel. Slike 
indeksvariablar kan til dømes vera forløpsid, innleggjingsdato og så vidare.

Eit anna problem oppstår dersom me har variablar av ulik type. I det enkle 
eksempelet med vekt og høgd går det bra, sidan begge desse er tal. Problemet 
oppstår når me til dømes har ein variabel for kva dato pasienten vart utskriven 
i tillegg. Då vil kolonnane `verdi_intern` og `verdi_ekstern` innehalda både 
tal og datoar.

Dette vert løyst ved å innføra fleire verdikolonnar, éin for kvar type variabel
i det opphavlege datasettet, samt ein kolonne `vartype` som seier kva type den 
aktuelle verdien er.

Til slutt gjev me alle kolonnane som ikkje er variablar frå det opphavlege 
datasettet prefikset *vld_*, kort for validering. Dette er eit reservert 
prefiks, og er med for å unngå problem med at namn på variablar er like som
namna på kolonnane i valideringsdatasettet. **Ingen** av variablane i det 
opphavlege datasettet kan altså ha namn som startar med *vld_*.

Eit eksempel på det endelege formatet vert då som dette:
...
`select(registerdata, -kjonn, -sjukehus)`. Når det er gjort er ein klar
...
valideringsdatasett. Det vil seie at alle variablar for alle pasientar/forløp 
er med. Talet på rader i valideringsdatasettet vert då lik talet på rader i
det opphavlege datasettet gongar talet på datavariablar. For eksempelet over, 
med 5 rader og 7 *datavariablar*, får ein eit valideringsdatasett med 35 rader. 
Her er dei fyrste 7 radene i dette valideringsdatasettet, som svarar til den fyrste 
rada i det opphavlege datasettet:
...
d_vld %>%
  head(7) %>%
  kbl(align = c("lrllrrrrrr")) %>%
  kable_classic("hover", font_size = 11) %>%
  row_spec(0, font_size = 9) # slik at tabellen ikkje skal verta for brei
```
...
#### Alle rader i tilfeldig rekkjefølgje
...
#### Alle rader med pasientar i tilfeldig rekkjefølgje
...
uttrekk_forlop = select(d_reg, pasid, dato_inn) %>% # pasid og dato_inn er indeksvariablane 
                                                    # bruk berre pasid dersom det skal velgast ut 
