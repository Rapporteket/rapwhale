test_that("Utdata skal seia (elementvis) om verdi1 er lik verdi2", {
  expect_identical(samanlikn_identisk(varnamn = rep("vekt", 5), 
                                      verdi1 = c(74, 74, 74, NA, NA), 
                                      verdi2 = c(74, 80, NA, 74, NA)), 
                   c(TRUE, FALSE, FALSE, FALSE, TRUE))
  
  expect_identical(samanlikn_identisk(varnamn = rep("dato", 5), 
                                      verdi1 = c(74, 74, 74, NA, NA) + Sys.Date(), 
                                      verdi2 = c(74, 80, NA, 74, NA) + Sys.Date()), 
                   c(TRUE, FALSE, FALSE, FALSE, TRUE))
  
  expect_identical(samanlikn_identisk(varnamn = rep("vekt", 7), 
                                      verdi1 = c("foo", "foo", "", "", "NA", NA, NA), 
                                      verdi2 = c("foo", "bar", NA, "", NA, "", NA)), 
                   c(TRUE, FALSE, FALSE, TRUE, FALSE, FALSE, TRUE))
  
  expect_identical(samanlikn_identisk(varnamn = rep("janei", 8), 
...
test_that("samanlikn_identisk() fungerer òg for vektorar med 1 eller 0 element", {
  # Eitt element
  expect_identical(samanlikn_identisk(varnamn = "vekt", 
                                      verdi1 = c(74), 
                                      verdi2 = c(74)), TRUE)
  
  # Null element
  expect_identical(samanlikn_identisk(varnamn = character(), 
                                      verdi1 = numeric(), 
                                      verdi2 = numeric()), logical())
  })
